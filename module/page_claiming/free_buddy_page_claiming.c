...
static void __attribute__((unused)) dumpinfo(struct page *requested_page) {
  printk(KERN_ERR "PHYDEBUG:pfn %08lx: headpfn %08lx count %d mapcount %d headcount %d flags %016lx headflags %016lx\n",
...
/* Try to claim the requested page, returns 0 for success or nonzero for error */
int do_try_claim_free_buddy_page(struct page *requested_page) {
  ...
  if (0 == page_count(compound_head(requested_page)) &&
    ...
    /* set_migratetype_isolate seems to set PageReserved which would cause */
    /* is_free_buddy_page() to fail */
    ...
    /* Note: This code proceeds even if isolating failed. The original never checked */
...
claim_free_buddy_page(struct page * requested) {
    ...
    if (likely(0 == requested_page_count) && PageBuddy(requested)) {
        ...
        area->nr_free--;
        mm_buddy_expand(zone, requested, order, current_order, area, migratetype);
